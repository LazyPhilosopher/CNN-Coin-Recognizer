...
class ResNet152V2Classifier_old:
...
class ResNet152V2Classifier:
    def __init__(self, input_shape=(128, 128, 3), num_classes=14, learning_rate=0.001, train_base=False):
    ...
    def _build_model(self):
        # Full image input
        full_image = Input(shape=self.input_shape, name="full_image")

        # Hue image input (grayscale, single channel)
        hue_image = Input(shape=(self.input_shape[0], self.input_shape[1], 1), name="hue_image")

        # Full image feature extraction using ResNet
        full_base = ResNet152V2(include_top=False, weights="imagenet", input_shape=self.input_shape, name="resnet152v2_full")
        full_base.trainable = self.train_base
        full_features = full_base(full_image)
        full_features = GlobalAveragePooling2D()(full_features)

        # Hue image feature extraction using a smaller ResNet
        hue_base = ResNet152V2(include_top=False, weights=None, input_shape=(self.input_shape[0], self.input_shape[1], 1), name="resnet152v2_hue")
        hue_base.trainable = self.train_base
        hue_features = hue_base(hue_image)
        hue_features = GlobalAveragePooling2D()(hue_features)

        # Concatenate features
        combined_features = Concatenate()([full_features, hue_features])
    ...
    def get_single_input_model(self):
        # Build a new model for inference that only uses full_image as input
        full_image = self.model.input["full_image"]  # Full image input
        full_features = self.model.get_layer("global_average_pooling2d").output  # Full image features
        x = Dense(512, activation="relu")(full_features)
        outputs = Dense(self.num_classes, activation="softmax")(x)

        inference_model = Model(inputs=full_image, outputs=outputs)
...
if __name__ == "__main__":
    ...
    try:
        ...
        train_dataset = tf.data.Dataset.load(f"trained/datasets/train_dataset_{testrun_name}.tfrecord")
        val_dataset = tf.data.Dataset.load(f"trained/datasets/val_dataset_{testrun_name}.tfrecord")
        ...
        save_dataset(val_dataset, f"trained/datasets/val_dataset_{testrun_name}.tfrecord")

        with open(f"trained/enumerations/train_enum_{testrun_name}.json", "w") as f:
            json.dump(enumerations_inv, f, indent=6)
    ...
    if not os.path.exists(model_path):
        ...
        single_input_model.save(model_path)

    else:
        print("Model already exists. Loading the model...")
        single_input_model = tf.keras.models.load_model(model_path)


    val_loss, val_accuracy = single_input_model.evaluate(val_dataset)
